...
# gets the average block time by sampling twice with a 60s sleep
def get_average_block_time(layerd_path):
    sleep_duration = 60
    """Calculate average block time by sampling twice with a 60s sleep interval"""
...
def sleep_box(duration=60):
    ...
    for i in range(duration):
        ...
        # Every 10 seconds, pick a new random joke
        within_cycle = elapsed % 10
        if within_cycle == 1 or within_cycle == 0:  # Start of new 10-second cycle
        ...
        if within_cycle == 0 or within_cycle <= 5:
            # Show question only (first 5 seconds of each 10-second cycle)
            question, _ = current_joke
            print(f"\033[91m🤔 {question}\033[0m")
            current_joke_lines = 1
        else:
            # Show question + answer (next 5 seconds)
